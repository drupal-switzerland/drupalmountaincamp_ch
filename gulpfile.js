...
gulp.task('sass', function() {
  var processors = [autoprefixer({ browsers: ['last 10 versions', 'ie 10'] })],
    processors_prod = [
  ...
  gulp
    ...
    .pipe(
      dev
        ? filter([path.css + '/**/*.css', '!' + path.css + '/animate.css'])
        : util.noop()
    )
...
gulp.task('images', function() {
  return gulp
    .src(path.theme + '/**/*.{png,gif,jpg,jpeg,svg}')
    .pipe(
      imagemin({
        progressive: true,
        interlaced: true,
...
gulp.task('help', function() {
  ...
  console.log(
    'gulp compile'.yellow + '\t\t\tcompile SASS for development enviroment'
  );
  console.log(
    'gulp compile --env=prod'.yellow +
      '\t\tcompile SASS for production enviroment'
  );
  console.log(
    'gulp svg'.yellow + '\t\t\trun Optimizes SVGs in beaker (image folder only)'
  );
  console.log('gulp images'.yellow + '\t\t\trun Optimizes Images in beaker');
  console.log(
    'gulp jshint'.yellow + '\t\t\ttool that helps to detect errors & problems'
  );
  console.log(
    'gulp iconfont'.yellow +
      '\t\t\tGenerate Iconfont from images/iconfont folder (SVG only)'
  );
