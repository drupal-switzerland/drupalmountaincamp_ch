 ...
 * @endcode
 */
$settings['hash_salt'] = 'G6aBpqYVKAscP1JwGZ3usn_iIp8kOTcvOc5_b8DoEvzk2rnsUEnWpdFw8T31fJePpjut0IGyEw';
...
// Ensure a fallback for CI.
$env = getenv('APP_ENV');
$env = !empty($env) ? $env : 'ci';
...
// Trust the direct peer (Lagoon ingress or the Nuxt SSR pod) so the
// X-Forwarded-Host/Proto headers are honored. Without this, GraphQL requests
// proxied through the frontend reach Drupal with Host "nginx:8080", the
// session cookie name no longer matches the public host and every request is
// treated as anonymous (which hides the Blokkli editor and admin toolbar).
$settings['reverse_proxy'] = TRUE;
